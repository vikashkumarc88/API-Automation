...
import com.api.model.PostMakePosts;
...
public class CreateBookingStepdefinition {
	...
	public void userMakesAPost() {
		String id = null;
		JSONObject Body = new JSONObject();
		context.response = context.requestSetup().body(Body.toString())
				.when().post(context.session.get("endpoint").toString());

		PostMakePosts postMakePosts = ResponseHandler.deserializedResponse(context.response, PostMakePosts.class);
		assertNotNull("Post not created", postMakePosts);
		LOG.info("id: "+postMakePosts.getId());
		context.session.put("id", postMakePosts.getId());
//		validatePostMakePostsData(new JSONObject(id), postMakePosts);
	}
	...
	private void validatePostMakePostsData(JSONObject id, PostMakePosts postMakePosts) {
		LOG.info(id);
		assertNotNull("ID missing", postMakePosts.getId());
	}
